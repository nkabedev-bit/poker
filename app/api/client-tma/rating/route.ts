...
export async function GET(request: Request) {
  ...
  // An imported period is served from the club's own table, month or season alike: those
  // totals are what the club published and what the players already saw. Only periods
  // with no imported sheet are counted from the games.
  const useArchive = Boolean(archivedRow);
