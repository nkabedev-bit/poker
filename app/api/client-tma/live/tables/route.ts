...
/**
 * The room table by table: who is still in at each one, and who went out from it.
 *
...
export async function GET(request: Request) {
  ...
  if (!live) return NextResponse.json({ tables: [], tournamentName: null });
  ...
  if (!context) return NextResponse.json({ tables: [], tournamentName: null });
  ...
  const tables = buildLiveTables(context.extras.players, {
  ...
  return NextResponse.json({ tables, tournamentName: live.tournamentName });
}
