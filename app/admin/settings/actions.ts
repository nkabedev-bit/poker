...
const settingsSchema = z.object({
  ...
  vipBuyIn: z.coerce.number().int().min(0).default(0),
  duoBuyIn: z.coerce.number().int().min(0).default(0),
  doubleRebuyPrice: z.coerce.number().int().min(0).default(0),
  bountyMode: z.enum(["off", "standard", "mystery", "dealer", "wanted", "progressive"]).default("off"),
  ...
  cardsEnabled: z.enum(["yes", "no"]).default("yes"),
  ...
  tournamentFormat: z.enum(["regular", "phoenix", "deepstack", "freeroll"]).default("regular"),
  tournamentPreset: z
...
export async function updateTournamentSettings(formData: FormData) {
  const parsed = settingsSchema.safeParse({
    ...
    vipBuyIn: formData.get("vipBuyIn") ?? 0,
    duoBuyIn: formData.get("duoBuyIn") ?? 0,
    doubleRebuyPrice: formData.get("doubleRebuyPrice") ?? 0,
    ...
    cardsEnabled: formData.get("cardsEnabled") === "no" ? "no" : "yes",
    ...
    tournamentPreset: formData.get("tournamentPreset") ?? "",
  ...
  if (!hasPublicEnv()) {
    ...
    await saveDemoExtras({
      settings: {
        ...
        vipBuyIn: parsed.data.vipBuyIn,
        duoBuyIn: parsed.data.duoBuyIn,
        doubleRebuyPrice: parsed.data.doubleRebuyPrice,
        bountyType: parsed.data.bountyMode === "off" ? "standard" : parsed.data.bountyMode,
        ...
        tournamentPreset: parsed.data.tournamentPreset,
