 ...
 * any of our code runs, and the player is only told the photo was not saved. The
 * profile shows the picture 320 pixels across, so the phone scales it down first and
 * sends a few hundred kilobytes instead.
 */

/** The longest side of the picture sent; the server cuts its 320-pixel square out of it. */
...
/**
 * The photo as a JPEG data URL no longer than `PHOTO_MAX_SIDE` on its longest side, or
 * null when the browser cannot open it — the caller then sends the file as it is, and
 ...
 */
export async function shrinkPhoto(file: Blob): Promise<string | null> {
  let url: string | null = null;

  try {
    url = URL.createObjectURL(file);
    const image = await loadImage(url);
    const { height, width } = fitWithin(image.naturalWidth, image.naturalHeight, PHOTO_MAX_SIDE);
