-- Persistent per-player achievement counters (across tournaments), keyed by telegram_id.
alter table public.client_bot_users
  add column if not exists games_played integer not null default 0,
  add column if not exists eliminations_count numeric not null default 0,
  add column if not exists top7_count integer not null default 0;

-- Accumulate achievement stats for every player who appeared in the final standings
-- of the given tournament. Idempotent per tournament instance via a statsCountedAt flag
-- stored inside tournament_extras.data.settings.
...
begin
  ...
  for item in select * from jsonb_array_elements(coalesce(extras_row.data->'players', '[]'::jsonb)) loop
    ...
    set
      ...
      top7_count = top7_count + (case when v_finish_place between 1 and 7 then 1 else 0 end)
