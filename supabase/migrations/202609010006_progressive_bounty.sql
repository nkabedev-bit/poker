-- Progressive Bounty: a knockout pays a head that grows with the victim's own
-- knockouts on the current bullet (base + step per knockout). The points themselves
-- ride the existing mystery-points pipeline; what the database has to keep is the
-- per-bullet knockout counter that prices the head:
--   * a killer's progressiveKnockouts grows by their bounty share;
--   * a re-entry resets the victim's counter to 0 (new bullet, base price again);
--   * bountyCount is untouched, so the tournament total still counts every knockout.
-- Cancelling an elimination reverses both halves; the victim's pre-knockout counter is
-- passed in from the bounty_log snapshot because the reset cannot be derived.

drop function if exists public.record_player_elimination(uuid, text, jsonb, numeric, numeric, boolean, boolean, boolean);
...
revoke all on function public.record_player_elimination(uuid, text, jsonb, numeric, numeric, boolean, boolean, boolean, boolean) from public;
grant execute on function public.record_player_elimination(uuid, text, jsonb, numeric, numeric, boolean, boolean, boolean, boolean) to authenticated, service_role;

drop function if exists public.cancel_player_elimination(uuid, text, integer, jsonb, numeric, boolean, jsonb, boolean);
