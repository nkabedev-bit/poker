-- Widen the VIP table (table 3) from 9 to 10 seats: standard tables take registration
-- numbers 1-20, table 3 takes 21-30. Mirrors VIP_REGISTRATION_NUMBER_MIN/MAX in
-- lib/player-registration-number.ts. Requires the tournament settings to allow at
-- least 30 numbers (3 tables x 10 players per table).
...
begin
  ...
  for candidate in 1..max_number loop
    if p_table_number = 3 then
      if candidate < 21 or candidate > 30 then
        continue;
      end if;
    else
      if candidate >= 21 and candidate <= 30 then
        continue;
      end if;
    end if;
