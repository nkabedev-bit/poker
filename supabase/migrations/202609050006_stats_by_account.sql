-- Medals and counters find a player by their club account.
--
-- The evening's standings were matched to a profile by Telegram id, which every player
-- had until the web door opened. A player who signs in with Yandex has none, so they
-- would have won a tournament and got no medal for it.
--
-- The seat now carries the account it was filled from, and that is what is counted
-- against. The Telegram id stays as the fallback for seats written before it existed,
